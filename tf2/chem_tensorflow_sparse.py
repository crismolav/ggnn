...
class SparseGGNNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['initial_node_representation'] = tf.compat.v1.placeholder(tf.float32, [None, h_dim],
                                                                          name='node_features')
        self.placeholders['adjacency_lists'] = [tf.compat.v1.placeholder(tf.int32, [None, 2], name='adjacency_e%s' % e)
                                                for e in range(self.num_edge_types)]
        self.placeholders['num_incoming_edges_per_type'] = tf.compat.v1.placeholder(tf.float32, [None, self.num_edge_types],
                                                                          name='num_incoming_edges_per_type')
        self.placeholders['graph_nodes_list'] = tf.compat.v1.placeholder(tf.int32, [None], name='graph_nodes_list')
        ...
        for layer_idx in range(len(self.params['layer_timesteps'])):
            with tf.compat.v1.variable_scope('gnn_layer_%i' % layer_idx):
                ...
                edge_weights = tf.nn.dropout(edge_weights, rate=1 - (self.placeholders['edge_weight_dropout_keep_prob']))
    ...
    def compute_final_node_representations(self) -> tf.Tensor:
        ...
        num_nodes = tf.shape(input=self.placeholders['initial_node_representation'], out_type=tf.int32)[0]
                ...
                for step in range(num_timesteps):
                    with tf.compat.v1.variable_scope('timestep_%i' % step):
                        ...
                        if self.params['use_propagation_attention']:
                            ...
                            message_attention_score_max_per_target = tf.math.unsorted_segment_max(data=message_attention_scores,
                            ...
                            message_attention_score_sum_per_target = tf.math.unsorted_segment_sum(data=message_attention_scores_exped,
                                                                                             segment_ids=message_targets,
                                                                                             num_segments=num_nodes)  # Shape [V]
                        ...
                        incoming_messages = tf.math.unsorted_segment_sum(data=messages,
                                                                    segment_ids=message_targets,
                        ...
                        if self.params['use_edge_msg_avg_aggregation']:
                            num_incoming_edges = tf.reduce_sum(input_tensor=self.placeholders['num_incoming_edges_per_type'],
    ...
    def gated_regression(self, last_h, regression_gate, regression_transform):
        ...
        graph_representations = tf.math.unsorted_segment_sum(data=gated_outputs,
