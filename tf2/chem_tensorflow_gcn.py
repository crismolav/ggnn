...
class SparseGCNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['initial_node_representation'] = tf.compat.v1.placeholder(tf.float32, [None, h_dim],
                                                                          name='node_features')
        self.placeholders['adjacency_list'] = tf.compat.v1.placeholder(tf.int64, [None, 2], name='adjacency_list')
        self.placeholders['adjacency_weights'] = tf.compat.v1.placeholder(tf.float32, [None], name='adjacency_weights')
        ...
        with tf.compat.v1.variable_scope('gcn_scope'):
    ...
    def compute_final_node_representations(self):
        with tf.compat.v1.variable_scope('gcn_scope'):
            ...
            num_nodes = tf.shape(input=self.placeholders['initial_node_representation'], out_type=tf.int64)[0]
            ...
            for layer_idx in range(self.params['num_timesteps']):
                scaled_cur_node_states = tf.sparse.sparse_dense_matmul(adjacency_matrix, cur_node_states)  # v x D
                ...
                if layer_idx < self.params['num_timesteps'] - 1:
                    ...
                    new_node_states = tf.nn.dropout(new_node_states, rate=1 - (self.placeholders['graph_state_keep_prob']))
