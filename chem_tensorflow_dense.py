...
class DenseGGNNChemModel(ChemModel):
    ...
    def gated_regression(self, last_h, regression_gate, regression_transform):
        ...
        softmax = tf.nn.softmax(masked_gated_outputs)
        output = tf.reduce_sum(masked_gated_outputs, axis = 1)                                              #
        max_output = tf.math.argmax(softmax, axis = 1)
    ...
    def process_raw_graphs(self, raw_data: Sequence[Any], is_training_data: bool, bucket_sizes=None) -> Any:
        ...
        for i, dd in enumerate(raw_data):
            ...
            if len(d['graph']) == 0:
                continue
            #c: this seems to be a "random" id chosen for the dictionary
            chosen_bucket_idx = np.argmax(bucket_sizes > max([v for e in d['graph']
                                                                for v in [e[0], e[2]]]))
            n_active_nodes = len(d["node_features"])
            # if dep_tree:
            #     chosen_bucket_size = n_active_nodes
            # else:
            #     chosen_bucket_size = bucket_sizes[chosen_bucket_idx]
            chosen_bucket_size = bucket_sizes[chosen_bucket_idx]
            #print("chosen_bucket_idx: %d"%chosen_bucket_idx)
            ...
            bucketed[chosen_bucket_idx].append({
                ...
                'labels': [d["targets"][task_id] + [0 for _ in range(chosen_bucket_size - n_active_nodes)] for task_id in self.params['task_ids']] ,
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                ...
                self.placeholders['target_values']: np.transpose(batch_data['labels']),
    ...
    def example_evaluation(self):
        ...
        for value in example_molecules.keys():
            ...
            self.print_results(example_molecules, value)

    def print_results(self, example_molecules, value):
        ...
        results = self.evaluate_one_batch(batch_data['init'], batch_data['adj_mat'])
